import { FC } from "react";
export const Resources: FC = () => {
    return (
        <div className={"my-4"}>
            <h1>Study resources</h1>
            <h2>Riichi Mahjong</h2>
            <p>
                <a href="https://docs.google.com/document/d/1e-vHoRqpJArIZwo8wttzzWtZRBbppPmtn-FZ-qevVn8">
                    Ruleset
                </a>
            </p>
            <p>
                <a href="https://docs.google.com/document/d/1SAMOHtm80w2xml9_VCZi3Mfq1XNrGfAamsZJvmpgi08">
                    Rules and Etiquette
                </a>
            </p>
            <h3>Tutorials</h3>
            <p>
                <a href="https://github.com/dainachiba/RiichiBooks/raw/master/RiichiBook1.pdf">
                    Riichi Book 1
                </a>
            </p>
            <p>
                <a href="https://drive.google.com/drive/folders/18hxO5DMVAqxSNV9VvpjAg6YjyPVAMzyS">
                    Cheat Sheet
                </a>
            </p>
            <p>
                {"Beginner's Luck "}
                <a href="http://beginners.biz/">Japanese</a>{" "}
                <a href="https://www.bilibili.com/read/readlist/rl45758">Chinese</a>
            </p>
            <p>
                {"Uzaku's tile efficiency "}
                <a href="https://drive.google.com/file/d/1ApHp2Dm-3dkEQTEAnmfTsk8J6OaH8d4G/view">
                    English
                </a>{" "}
                <a href="https://www.bilibili.com/read/readlist/rl509592">Chinese</a>
            </p>
            <p>
                {"WWYD 300 "}
                <a href="https://files.riichi.moe/mjg/books%20(en)/300%20Established%20Practice%20Which%20to%20cut%20%28wwyd-chan%201%29.pdf">
                    English
                </a>{" "}
                <a href="https://www.bilibili.com/read/readlist/rl380536">Chinese</a>
            </p>
            <p>
                {"WWYD 301 "}
                <a href="https://drive.google.com/file/d/1HQcoZ96XSVnEzy_ABNOxkxHLYQgYAeWr/view">
                    English
                </a>{" "}
                <a href="https://www.bilibili.com/read/readlist/rl493079">Chinese</a>
            </p>
            <h2>Hong Kong Mahjong</h2>
            <p>
                <a href="https://docs.google.com/document/d/1luP0TJKwsac2XzWv9D8bNesYPI4ZJO4zaThDY2g2Z7U">
                    Charter
                </a>
            </p>
        </div>
    );
};
